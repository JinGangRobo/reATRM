        ...
        if (number) {
            if (CFGetTypeID(number) == CFNumberGetTypeID()) {
                bool success = CFNumberGetValue(
                    static_cast<CFNumberRef>(number), kCFNumberSInt16Type, &result);
...
vector<PortInfo> serial::list_ports(void) {
    ...
    while ((serial_port = IOIteratorNext(serial_port_iterator))) {
        ...
        if (vendor_id && product_id) {
            ...
            int ret = snprintf(
                cstring, HARDWARE_ID_STRING_LENGTH, "USB VID:PID=%04x:%04x SNR=%s", vendor_id,
                product_id, serial_number.c_str());
