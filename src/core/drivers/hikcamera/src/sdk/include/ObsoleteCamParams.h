...
typedef struct _MV_IMAGE_BASIC_INFO_ {
    ...
    unsigned int nSupportedPixelFmtNum; ///< [OUT]  \~chinese 支持的像素格式种类    \~english
...
typedef struct _MV_CC_BAYER_NOISE_ESTIMATE_PARAM_T_ {
    ...
    unsigned int nNoiseThreshold; ///< [IN]  \~chinese 噪声阈值(0-4095)               \~english
                                  ///< Noise Threshold
...
typedef struct _MV_CC_BAYER_SPATIAL_DENOISE_PARAM_T_ {
    ...
    unsigned int nDstBufSize; ///< [IN]  \~chinese 提供的输出缓冲区大小           \~english Provided
                              ///< output buffer size
    unsigned int
        nDstBufLen; ///< [OUT] \~chinese 输出降噪后的数据长度           \~english Output data length

    MV_CC_BAYER_NOISE_PROFILE_INFO stNoiseProfile; ///< [IN]  \~chinese 降噪特性信息(来源于噪声估计)
                                                   ///< \~english Denoise profile
    unsigned int nDenoiseStrength; ///< [IN]  \~chinese 降噪强度(0-100)                \~english
                                   ///< nDenoise Strength
    unsigned int nSharpenStrength; ///< [IN]  \~chinese 锐化强度(0-32)                 \~english
                                   ///< Sharpen Strength
    unsigned int
...
typedef struct _MV_CC_CLUT_PARAM_T_ {
    ...
    unsigned int
        nCLUTBufLen;         ///< [IN]  \~chinese
                     ///< 量化CLUT缓存大小(nCLUTSize*nCLUTSize*nCLUTSize*sizeof(int)*3)  \~english
                     ///< CLUT buffer length(nCLUTSize*nCLUTSize*nCLUTSize*sizeof(int)*3)
...
typedef struct _MV_CC_SHARPEN_PARAM_T_ {
    ...
    unsigned int nSharpenAmount; ///< [IN]  \~chinese 锐度调节强度，[0,500]  \~english Sharpen
                                 ///< amount,[0,500]  //   [nSharpenAmount 作废， 使用
                                 ///< nSharpenPosAmount & nSharpenNegAmount 替代 ]
    unsigned int
        nSharpenRadius; ///< [IN]  \~chinese 锐度调节半径(半径越大，耗时越长)，[1,21]   \~english
                        ///< Sharpen radius(The larger the radius, the longer it takes),[1,21]
    unsigned int nSharpenThreshold; ///< [IN]  \~chinese 锐度调节阈值，[0,255]  \~english Sharpen
                                    ///< threshold,[0,255]
...
typedef struct _MV_CC_NOISE_ESTIMATE_PARAM_T_ {
    ...
    ///< \~chinese Bayer域噪声估计参数，Mono8/RGB域无效     \~english Bayer Noise estimate
    ///< param,Mono8/RGB formats are invalid
    unsigned int nNoiseThreshold; ///< [IN]  \~chinese 噪声阈值[0,4095]       \~english Noise
                                  ///< threshold[0,4095]
    ///< \~chinese 建议值:8bit,0xE0;10bit,0x380;12bit,0xE00     \~english Suggestive
    ///< value:8bit,0xE0;10bit,0x380;12bit,0xE00
...
typedef struct _MV_CC_SPATIAL_DENOISE_PARAM_T_ {
    ...
    unsigned int nNoiseProfileLen; ///< [IN]  \~chinese 输入噪声特性长度       \~english Input Noise
                                   ///< Profile length

    ///< \~chinese Bayer域空域降噪参数，Mono8/RGB域无效     \~english Bayer Spatial denoise
    ///< param,Mono8/RGB formats are invalid
    unsigned int nBayerDenoiseStrength; ///< [IN]  \~chinese 降噪强度[0,100]        \~english
...
/// \~chinese 某个节点对应的子节点个数最大值    \~english The maximum number of child nodes
...
/// \~chinese 节点描述字段最大长度              \~english The maximum length of the node description
/// field
...
/// \~chinese 每个已经实现单元的名称长度        \~english The length of the name of each unit that
...
typedef struct _MV_XML_FEATURE_Integer_ {
    ...
    char strDescription[MV_MAX_XML_DISC_STRLEN_C]; ///< \~chinese 目前暂不支持     \~english NOT
                                                   ///< SUPPORT NOW
...
typedef struct _MV_XML_FEATURE_Enumeration_ {
    ...
    char strCurrentSymbolic[MV_MAX_XML_SYMBOLIC_STRLEN_C]; ///< \~chinese 当前Symbolic索引 \~english
