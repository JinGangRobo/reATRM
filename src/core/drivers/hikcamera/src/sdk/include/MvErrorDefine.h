...
#define MV_E_PRECONDITION \
    0x80000008 ///< \~chinese 前置条件有误，或运行环境已发生变化   \~english Precondition error, or
               ///< running environment changed
...
#define MV_E_ABNORMAL_IMAGE \
    0x8000000B ///< \~chinese 异常图像，可能是丢包导致图像不完整   \~english Abnormal image, maybe
               ///< incomplete image because of lost packet
...
#define MV_E_NOT_IMPLEMENTED \
    0x80000200 ///< \~chinese 命令不被设备支持         \~english The command is not supported by
               ///< device
#define MV_E_INVALID_ADDRESS \
    0x80000201 ///< \~chinese 访问的目标地址不存在     \~english The target address being accessed
               ///< does not exist
...
#define MV_E_BUSY \
    0x80000204 ///< \~chinese 设备忙，或网络断开       \~english Device is busy, or network
               ///< disconnected
#define MV_E_PACKET \
...
#define MV_E_UPG_CONFLICT \
    0x80000402 ///< \~chinese 升级冲突（设备已经在升级了再次请求升级即返回此错误） \~english
               ///< Upgrading conflicted (repeated upgrading requests during device upgrade)
#define MV_E_UPG_INNER_ERR \
    0x80000403 ///< \~chinese 升级时设备内部出现错误   \~english Camera internal error during
               ///< upgrade
