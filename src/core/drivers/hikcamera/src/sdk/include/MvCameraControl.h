...
/**************************Part1 ch: 相机的控制和取流接口 | en: Camera control and
 * streaming******************************************/
...
/********************************************************************/ /**
  ...
  *  @param  nTLayerType                 [IN]            枚举传输层, 参数定义参见CameraParams.h定义,
  如: #define MV_GIGE_DEVICE 0x00000001 GigE设备
  *  @param  pstDevList                  [IN][OUT]       设备列表
  *  @return 成功，返回MV_OK；错误，返回错误码
  *  @remarks
  设备列表的内存是在SDK内部分配的，多线程调用该接口时会进行设备列表内存的释放和申请,建议尽量避免多线程枚举操作。
  *  @remarks
  参数枚举传输层，适配传入MV_GIGE_DEVICE、MV_1394_DEVICE、MV_USB_DEVICE、MV_CAMERALINK_DEVICE；MV_GIGE_DEVICE该参数
              传出所有GiGE相关的设备信息（包含虚拟GiGE和GenTL下的GiGE设备），MV_USB_DEVICE该参数传出所有USB设备，包含虚拟USB设备。

  *  @~english
  *  @brief  Enumerate Device
  *  @param  nTLayerType                 [IN]            Enumerate TLs, Refer to the
  'CameraParams.h' for parameter definitions, for example, #define MV_GIGE_DEVICE 0x00000001
  ...
  *  @remarks The memory of the device list is allocated within the SDK. When the interface is
  invoked by multiple threads, the memory of the device list will be released and applied It is
  recommended to avoid multithreaded enumeration operations as much as possible.
  *  @remarks Transmission layer of enumeration, param only include
  MV_GIGE_DEVICE、MV_1394_DEVICE、MV_USB_DEVICE、MV_CAMERALINK_DEVICE; MV_GIGE_DEVICE can output
...
/********************************************************************/ /**
  ...
  *  @param  nTLayerType                 [IN]            Transmission layer of enumeration, , Refer
  to the 'CameraParams.h' for parameter definitions, for example, #define MV_GIGE_DEVICE 0x00000001
  *  @param  pstDevList                  [IN][OUT]       Device list
...
/********************************************************************/ /**
  ...
  *  @param  nTLayerType                 [IN]            枚举传输层（区分每一种传输层类型，不耦合）,
  参数定义参见CameraParams.h定义, 如: #define MV_GIGE_DEVICE 0x00000001 GigE设备
  *  @param  pstDevList                  [IN][OUT]       设备列表
  ...
  *  @param  nTLayerType                 [IN]            Transmission layer of enumeration(All layer
  protocol type can input), Refer to the 'CameraParams.h' for parameter definitions, for example,
  ...
  *  @remarks The memory of the device list is allocated within the SDK. When the interface is
  invoked by multiple threads, the memory of the device list will be released and applied. It is
  recommended to avoid multithreaded enumeration operations as much as possible. strManufacturerName
  can be passed in NULL,if NULL is passed in, it will return the sorted list of all devices. If it
  is not NULL,it will only return the sorted list of the specified manufacturer's devices.
  ************************************************************************/
...
/********************************************************************/ /**
  ...
  *  @param  nAccessMode                 [IN]            访问权限,参数定义参见CameraParams.h定义,
  如:#define MV_ACCESS_Exclusive 1
  ...
  *  @param  nAccessMode                 [IN]            Access Right, Refer to the 'CameraParams.h'
  for parameter definitions, for example, #define MV_ACCESS_Exclusive 1
...
/********************************************************************/ /**
  ...
  *  @return  成功，返回MV_OK；错误，返回错误码
  ************************************************************************/
MV_CAMCTRL_API int __stdcall MV_CC_EnumInterfaces(
    IN unsigned int nTLayerType, IN OUT MV_INTERFACE_INFO_LIST* pInterfaceInfoList);
...
/********************************************************************/ /**
  ...
  *  @return Success, return MV_OK. Failure, return error code
  *  @remarks Find specific device and connect according to set device parameters.When calling the
  interface, the input of nAccessMode and nSwitchoverKey is optional, and the device access mode is
  exclusive. Currently the device does not support the following preemption modes:
              MV_ACCESS_ExclusiveWithSwitch, MV_ACCESS_ControlWithSwitch,
  MV_ACCESS_ControlSwitchEnableWithKey; For USB3Vision device, nAccessMode, nSwitchoverKey are
  invalid. This Interface support open without enumeration by GEV device，USB device and GenTL
...
/********************************************************************/ /**
  ...
  *  @return Success, return MV_OK. Failure, return error code
  *  @remarks After MV_CC_CreateHandle, call this interface to set image data callback
  function.There are two available image data acquisition modes, and cannot be used together: Mode
  1: Call MV_CC_RegisterImageCallBack to set image data callback function, and then
  callMV_CC_StartGrabbing to start acquiring. The acquired image data will return in the set
  callback function. Mode 2: Call MV_CC_StartGrabbing to start acquiring, and then call
  ...
  upper layer application according to frame rate. This API is not supported by CameraLink device.
  ***********************************************************************/
...
/********************************************************************/ /**
  ...
  *  @return 成功，返回MV_OK；错误，返回错误码
  *  @remarks
  通过该接口可以设置图像数据回调函数，在MV_CC_CreateHandle之后即可调用。图像数据采集有两种方式，两种方式不能复用：
  ...
  *  @remarks Before calling this API to set image data callback function, you should call this API
  MV_CC_CreateHandle.There are two image acquisition modes, the two modes cannot be reused: Mode 1:
  Call MV_CC_RegisterImageCallBackForRGB to set RGB24 format image data callback function, and then
  call MV_CC_StartGrabbing to start acquisition, the collected image data will be returned in the
  configured callback function. Mode 2: Call MV_CC_StartGrabbing to start acquisition, and the call
  MV_CC_GetImageForRGB repeatedly in application layer to get frame data with RGB24 format. When
  getting frame data, the upper application program should control the frequency of calling this API
  according to frame rate. This API is not supported by CameraLink device.
  ***********************************************************************/
...
/********************************************************************/ /**
  ...
  *  @remarks Before calling this API to set image data callback function, you should call this API
  MV_CC_CreateHandle.There are two image acquisition modes, the two modes cannot be reused: Mode 1:
  Call MV_CC_RegisterImageCallBackForBGR to set RGB24 format image data callback function, and then
  call MV_CC_StartGrabbing to start acquisition, the collected image data will be returned in the
  configured callback function. Mode 2: Call MV_CC_StartGrabbing to start acquisition, and the call
  MV_CC_GetImageForBGR repeatedly in application layer to get frame data with BGR24 format. When
  getting frame data,the upper application program should control the frequency of calling this API
  according to frame rate. This API is not supported by CameraLink device.
  ***********************************************************************/
...
/********************************************************************/ /**
  ...
  *  @return 成功，返回MV_OK；错误，返回错误码
  *  @remarks
  每次调用该接口，将查询内部缓存是否有数据，如果有数据则转换成RGB24格式返回，如果没有数据则返回错误码。
  ...
  *  @remarks Each time the API is called, the internal cache is checked for data. If there is data,
  ...
  the data frame rate is too high. Before calling this API to get image data frame, call
  MV_CC_StartGrabbing to start image acquisition. This API can get frame data actively, the upper
  layer program should control the frequency of calling this API according to the frame rate. This
  API is not supported by CameraLink device.
  ***********************************************************************/
...
/********************************************************************/ /**
  ...
  *  @return 成功，返回MV_OK；错误，返回错误码
  *  @remarks
  每次调用该接口，将查询内部缓存是否有数据，如果有数据则转换成BGR24格式返回，如果没有数据则返回错误码。
...
/********************************************************************/ /**
  ...
  *  @remarks Before calling this API to get image data frame, you should call MV_CC_StartGrabbing
  ...
  stability, which can be used in the situation with high stability requirement. This API and
  MV_CC_FreeImageBuffer should be called in pairs, after processing the acquired data, you should
  call MV_CC_FreeImageBuffer to release the data pointer permission of pFrame. This interface is
...
/********************************************************************/ /**
  ...
  *  @remarks This API and MV_CC_GetImageBuffer should be called in pairs, before calling
  ...
  the permission. Compared with API MV_CC_GetOneFrameTimeout this API has higher efficiency of image
  acquisition. The max. number of nodes can be outputted is same as the "nNum" of API
  MV_CC_SetImageNodeNum, default value is 1. This API is not supported by CameraLink device. This
  API is supported by both USB3 vision camera and GigE camera.
  **********************************************************************/
...
/********************************************************************/ /**
  ...
  *  @remarks Before calling this API to get image data frame, call MV_CC_StartGrabbing to start
  ...
  which can be used in the situation with high stability requirement. Both the USB3Vision and GIGE
  camera can support this API. This API is not supported by CameraLink device.
  ***********************************************************************/
...
/********************************************************************/ /**
  ...
  *  @remarks This interface allows user to clear the unnecessary images from the buffer memory
  without stopping acquisition. This interface allows user to clear previous data after switching
...
/********************************************************************/ /**
  ...
  *  @param  handle                      [IN]            Device handle
  *  @param  pnValidImageNum             [IN][OUT]       The number of valid images in the current
  image buffer
  *  @return Success, return MV_OK. Failure, return error code
  *  @remarks
  ***********************************************************************/
...
/********************************************************************/ /**
  ...
  *  @return 成功，返回MV_OK；错误，返回错误码
  *  @remarks
  该接口支持PixelType_Gvsp_RGB8_Packed，PixelType_Gvsp_BGR8_Packed，PixelType_Gvsp_Mono8三种像素格式的渲染宽高大小至int类型
  *           其余像素格式渲染仅支持宽高至short
  ...
  *  @param  pstDisplayInfo              [IN]            Frame Info
  *  @return Success, return MV_OK. Failure, return error code
  *  @remarks This API support rendering in three pixel
  formats:PixelType_Gvsp_RGB8_Packed,PixelType_Gvsp_BGR8_Packed and PixelType_Gvsp_Mono8,width and
  height to int.
...
/********************************************************************/ /**
  ...
  *  @remarks Call this interface to set the number of SDK internal image buffer nodes. The
  interface should be called before calling MV_CC_StartGrabbing for capturing. In the SDK, it
  defaults to 1 node. For CameraLink devices: This interface does not support serial connection, but
  supports GenTL connection.
  ***********************************************************************/
...
/********************************************************************/ /**
  ...
  *  @remarks This interface is set by four image acquisition approaches, the user may choose one as
  ...
  of ImageNodeNum is 1, If the user usesMV_CC_SetImageNodeNuminterface to set up
  OutputQueueSize,when the value of OutputQueueSize is set to be 1, the function will be same as
  LatestImagesOnly; if the value of OutputQueueSize is set to be ImageNodeNum, the function will be
  same as OneByOne. UpcomingImage:Ignore all images in output cache list when calling image
...
/********************************************************************/ /**
  *  @~chinese
  *  @brief
  设置输出缓存个数（该接口只支持windows平台，只有在MV_GrabStrategy_LatestImages策略下才有效，范围：1-ImageNodeNum）
  *  @param  handle                      [IN]            设备句柄
  ...
  *  @~english
  *  @brief  Set The Size of Output Queue(This interface only supports windows platform, Only work
  under the strategy of MV_GrabStrategy_LatestImages，rang：1-ImageNodeNum)
  *  @param  handle                      [IN]            Device handle
...
/********************************************************************/ /**
  ...
  *  @return Success, return MV_OK. Failure, return error code
  *  @remarks The API support users to access device information after opening the device，don't
...
/********************************************************************/ /**
  ...
  *  @remarks Input required information type (specify nType in structure MV_ALL_MATCH_INFO) in the
  interface and get corresponding information (return in pInfo of structure MV_ALL_MATCH_INFO) The
  calling precondition of this interface is determined by obtained information type. Call after
  enabling capture to get MV_MATCH_TYPE_NET_DETECT information of GigE device, and call after
...
/********************************************************************/ /**
  ...
  *  @return Success, return MV_OK. Failure, return error code
  *  @remarks Call this interface after connecting the device to obtain the symbol corresponding to
  the value of the specified node of Enum type.
  ************************************************************************/
...
/********************************************************************/ /**
  ...
  *  @remarks When pData is NULL or nDataSize than the actual XML file hours, do not copy the data,
  returned by pnDataLen XML file size. When pData is a valid cache address and the cache is large
...
/********************************************************************/ /**
  ...
  *  @return 成功，返回MV_OK；错误，返回错误码
  *  @remarks
  该接口直接使用缓存数据，进行读写操作，避免直接操作文件出现无权限的问题,是MV_CC_FileAccessWrite的扩展接口
...
/********************************************************************/ /**
  ...
  *  @remarks Call this API to send the upgrade firmware to the device for upgrade.
              This API will wait for return until the upgrade firmware is sent to the device, this
  response may take a long time. For CameraLink device, it keeps sending upgrade firmware
  continuously.
  ************************************************************************/
...
/********************************************************************/ /**
  ...
  *  @remarks Call this interface after the device is opened by MV_CC_OpenDevice.
              When device is exceptionally disconnected, the exception message can be obtained from
 callback function. For Disconnected GigE device, first call MV_CC_CloseDevice to shut device, and
 then call MV_CC_OpenDevice to reopen the device.
 ************************************************************************/
...
/********************************************************************/ /**
 ...
 acquisition, exposure, and so on This API is not supported by CameraLink device.
 ************************************************************************/
...
/*******************Part5 ch: 仅GigE设备支持的接口 | en: Only support GigE
 * interface*******************/
...
/********************************************************************/ /**
  ...
  *  @~english
  *  @brief  Set enumerate device timeout,only support GigE,range 1 - UINT_MAX,(the minimum value is
  greater than or equal to 1,maximum value less than UINT_MAX)
  *  @param  nMilTimeout                 [IN]            time out,default 100ms
  *  @return Success, return MV_OK. Failure, return error code
  *  @remarks Before calling enum device interfaces,call MV_GIGE_SetEnumDevTimeout to set max
  timeout,can reduce the maximum timeout to speed up the enumeration of GigE devices.
...
/********************************************************************/ /**
 ...
 supported by GigEVision camera. If device is in DHCP status, after calling this API to force
...
/********************************************************************/ /**
  ...
  *  @remarks To get optimized packet size, for GigEVision device is SCPS
              and it is the size of a packet transported on the network. The interface should be
  called after MV_CC_OpenDevice and before MV_CC_StartGrabbing. This API is not supported by
  CameraLink device and U3V device. This interface does not support GenTL devices (protocol not
  supported). If a network camera is added in GenTL mode, it is recommended to configure
  GevSCPSPacketSize according to the actual network situation,or 1500.
  ************************************************************************/
...
/********************************************************************/ /**
  ...
  *  @param  handle                      [IN]            Device handle
  *  @param  pnRetryTimes                [IN][OUT]       The max times to retry resending lost
  packets
  *  @return Success, return MV_OK. Failure, return error code
  *  @remarks This interface MUST be called after enabling resending lost packets by calling
  MV_GIGE_SetResend,
...
/********************************************************************/ /**
  ...
  *  @param  handle                      [IN]            Device handle
  *  @param  pnMillisec                  [IN][OUT]       The time interval between same resend
  requests
  *  @return Success, return MV_OK. Failure, return error code
  *  @remarks This interface MUST be called after enabling resending lost packets by calling
  MV_GIGE_SetResend,
...
/***********************************************************************************************************/ /**
  ...
  *  @remarks This API is supported only by CameraLink device.
              This API support calls when devices are not connected. But it is necessary to connect
 to the device first when accessing a CameraLink Device through the GenTL protocol. Due to
 hardware/system/external interference and other factors, configuring a high baud rate may cause
 abnormal communication. It is recommended to configure a baud rate of less than 115200
 ************************************************************************************************************/
...
/********************************************************************/ /**
  ...
  *  @brief  获取设备波特率
  *  @param  handle                      [IN]            设备句柄
  *  @param  pnCurrentBaudrate           [IN][OUT]
  ...
  *  @param  handle                      [IN]            Device handle
  *  @param  pnCurrentBaudrate           [IN][OUT]       Return pointer of baud rate to user. Refer
...
/********************************************************************/ /**
  ...
  *  @param  handle                      [IN]            Device handle
  *  @param  pnBaudrateAblity            [IN][OUT]       Return pointer of the supported baudrates
...
/*******************Part7 ch: 仅U3V设备支持的接口 | en: Only support U3V device
 * interface*******************/
...
/********************************************************************/ /**
 ...
 Byte，默认为1M，rang：>=0x400，建议最大值：[windows] rang <= 0x400000；[Linux] rang <= 0x200000
  *  @return 成功,返回MV_OK,失败,返回错误码
  *  @remarks
 ...
 size，Byte，default：1M，rang：>=0x400，Recommended maximum: [windows] rang <= 0x400000; [Linux]
 rang <= 0x200000
  *  @return Success, return MV_OK. Failure, return error code
...
/********************************************************************/ /**
  ...
  *  @~english
  *  @brief  Register exception stream callBack, call after open device (only support U3V Camera,
 don't support GenTL Device)
  *  @param  handle                      [IN]            Device handle
...
/********************************************************************/ /**
  ...
  *  @return Success, return MV_OK. Failure, return error code
  *  @remarks This interface is used to set the current number of U3V event nodes. default to 5
 nodes.
 ************************************************************************/
...
/********************************************************************/ /**
  ...
  *  @~english
  *  @brief  Set U3V Synchronisation timeout,range is 1000 ~ UINT_MAX(minimum value greater than
 1000,maximum value less than UINT_MAX)
  *  @param  handle               [IN]            Device handle
  *  @param  nMills               [IN]            set synchronisation timeout(ms),default 1000ms
  *  @return Success, return MV_OK. Failure, return error code
  *  @remarks Increasing the SetSyncTimeOut can compatible with some camera configuretion parameters
 very slow,more than 1000ms
 ************************************************************************/
...
/********************************************************************/ /**
  ...
  *  @remarks
  通过该接口可以将从设备采集到的原始图像数据转换成JPEG或者BMP等格式并存放在指定内存中，然后用户可以将转换之后的数据直接保存成图片文件。
              ...
              该接口支持图像 宽、高、总长最大至 UINT_MAX, 其中MV_CC_SaveImageEx2支持
  宽、高、总长最大至 USHRT_MAX JPEG格式最大支持宽高为65500
...
/********************************************************************/ /**
  ...
  *  @remarks 该接口支持BMP/JPEG/PNG/TIFF。
              该接口支持图像 宽、高、总长最大至 UINT_MAX, 其中MV_CC_SaveImageToFile支持
  ...
  *  @remarks This API support BMP/JPEG/PNG/TIFF.
              Comparing with the API MV_CC_SaveImageToFile, this API support the parameter
  nWidth/nHeight/nDataLen to UINT_MAX. JPEG format supports a maximum width and height of 65500
  ************************************************************************/
...
/********************************************************************/ /**
  ...
  *  @remarks 通过将接口可以将从设备采集到的原始图像数据转换成用户所需的像素格式并存放在指定内存中。
              ...
              该接口支持图像 宽、高、总长最大至 UINT_MAX, 其中MV_CC_ConvertPixelType支持
  宽、高、总长最大至 USHRT_MAX
  ...
  *  @remarks This API is used to transform the collected original data to pixel format and save to
  ...
  set callback function, and get a frame of image data, then call this API to transform the format.
              Comparing with the API MV_CC_ConvertPixelType, this API support the parameter
...
/********************************************************************/ /**
  ...
  *  @return 成功，返回MV_OK；错误，返回错误码
  *  @remarks
  设置内部图像转换接口的Bayer插值算法类型参数，MV_CC_ConvertPixelTypeEx、MV_CC_GetImageForRGB/BGR接口内部使用的插值算法是该接口所设定的。
...
/********************************************************************/ /**
  ...
  *  @remarks
  设置内部图像转换接口的贝尔插值平滑使能参数，MV_CC_ConvertPixelTypeEx、MV_CC_SaveImageEx3接口内部使用的插值算法是该接口所设定的。
  ...
  *  @return Success, return MV_OK. Failure, return error code
  *  @remarks Set the bell interpolation filter type parameters of the internal image conversion
  interface, and the interpolation algorithm used in the MV_CC_ConvertPixelTypeEx and
  MV_CC_SaveImageEx3 interfaces is set by this interface.
  ************************************************************************/
...
/********************************************************************/ /**
  ...
  *  @return 成功，返回MV_OK；错误，返回错误码
  *  @remarks
  设置该值后，在调用MV_CC_ConvertPixelTypeEx、MV_CC_SaveImageEx3、MV_CC_SaveImageToFileEx接口将Bayer8/10/12/16格式转成RGB24/48，
  ...
  *  @return Success, return MV_OK. Failure, return error code
  *  @remarks After setting the value, it works when calling
...
/********************************************************************/ /**
  ...
  *  @param   handle                           [IN] 设备句柄
  *  @param   MvGvspPixelType enSrcPixelType   [IN]
  像素格式,支持PixelType_Gvsp_Mono8,Bayer8/10/12/16
  *  @param   fGammaValue                      [IN] Gamma值:0.1 ~ 4.0
  *  @return  成功，返回MV_OK；错误，返回错误码
  *  @remarks
  设置Mono8的gamma值后，在调用MV_CC_ConvertPixelTypeEx接口将Mono8转成Mono8时gamma值起效。
  *  @remarks
  设置Bayer8/10/12/16的gamma值后，在调用MV_CC_ConvertPixelTypeEx、MV_CC_SaveImageToFileEx、MV_CC_SaveImageEx3接口将Bayer8/10/12/16转RGB24/48，
  RGBA32/64，BGR24/48，BGRA32/64时gamma值起效。
  *  @remarks 该接口兼容MV_CC_SetBayerGammaValue接口，新增支持Mono8像素格式

  *  @~english
  *  @brief  Set Gamma value
  *  @param  handle                           [IN]            Device handle
  *  @param  MvGvspPixelType enSrcPixelType   [IN]            PixelType,support
  PixelType_Gvsp_Mono8,Bayer8/10/12/16
  *  @param  fGammaValue                      [IN]            Gamma value:0.1~ 4.0
  *  @remarks After setting the gamma of Mono8 ，the gamma value takes effect when calling
  MV_CC_ConvertPixelTypeEx converts Mono8 to Mono8.
  *  @remarks After setting the gamma of Bayer8/10/12/16, the gamma value takes effect when calling
  MV_CC_ConvertPixelTypeEx\MV_CC_SaveImageToFileEx\MV_CC_SaveImageEx3 converts Bayer8/10/12/16 to
  RGB24/48,RGBA32/64,BGR24/48,BGRA32/64.
...
/********************************************************************/ /**
  ...
  *  @return 成功，返回MV_OK；错误，返回错误码
  *  @remarks
  设置该信息后，在调用MV_CC_ConvertPixelTypeEx、MV_CC_SaveImageEx3、MV_CC_SaveImageToFileEx接口将Bayer8/10/12/16格式转成RGB24/48，
  RGBA32/64，BGR24/48，BGRA32/64时Gamma值起效。
...
/********************************************************************/ /**
  ...
  *  @return 成功，返回MV_OK；错误，返回错误码
  *  @remarks
  开启CCM并设置CCM矩阵后，在调用MV_CC_ConvertPixelTypeEx、MV_CC_SaveImageEx3接口将Bayer8/10/12/16格式转成RGB24/48，
  RGBA32/64，BGR24/48，BGRA32/64时起效。
...
/********************************************************************/ /**
  ...
  *  @remarks
  将从相机中取到的无损压缩码流解码成裸数据，同时支持解析当前相机实时图像的水印信息（如果输入的无损码流不是当前相机或者不是实时取流的，则水印解析可能异常）;
                          若解码失败，请检查以下情况：（1）需要CPU支持 SSE
  AVX指令集（2）若当前帧异常（丢包等）,可能导致解码异常（3）相机出图异常， 即使不丢包也会异常
  ...
  *  @return Success, return MV_OK. Failure, return error code
  *  @remarks Decode the lossless compressed data from the camera into raw data，At the same time,
  it supports parsing the watermark information of the real-time image of the current camera (if the
  input lossless code stream is not the current camera or is not real-time streaming, the watermark
  parsing may be abnormal); If decoding fails, please check the following: (1) The CPU is required
  to support the SSE AVX instruction set. (2) If the current frame is abnormal (packet loss, etc.),
...
/********************************************************************/ /**
  ...
  *  @remarks
  限制：在同一线程中多相机同时调用该接口，只能打开当前一个GUI界面，需要关闭当前相机GUI界面后，才可打开另一个相机的GUI界面（后续版本优化）
                          该接口仅支持windows平台
  ...
  *  @param  handle                      [IN]            Device handle
  *  @return Success, return MV_OK, Failure, return error code.
  *  @remarks After connecting to device through MV_CC_OpenDevice, use this interface to get or set
  device params.
  *  @remarks Limit: calling this interface multiple times in the same thread can only open one GUI
  interface. You need to wait until the previous GUI interface is closed before opening the next GUI
  interface.(Subsequent version optimization) This interface only supports windows platform.
  ************************************************************************/
...
/********************************************************************/ /**
  ...
  *  @return Success, return MV_OK, Failure, return error code.
  *  @remarks Image segmentation supports any pixel format. Image segmentation should be used with
  the "MultiLightControl" node of the linear array camera. This node can set multiple different
  exposure values, such as MultiLightControl=2, The camera will overlap and merge two images
  corresponding to two different exposure values into one image (the actual height is the height of
  the two images) and send it to the upper application. Call the interface and pass in nExposureNum
  is two. One image sent by the camera can be divided into two images, each of which corresponds to
  an exposure value. If an ordinary camera is used or the "MultiLightControl" node of the linear
  array camera is not turned on, the image segmentation is meaningless, but the image is divided
